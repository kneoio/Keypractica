...
public class OrgCategoryController extends AbstractSecuredController<OrgCategory, OrgCategoryDTO> {
    ...
    @Path("/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    public Uni<Integer> delete(@Pattern(regexp = UUID_PATTERN) @PathParam("id") String id, @Context ContainerRequestContext requestContext) throws UserNotFoundException {
