...
import io.kneo.qtracker.repository.table.OwnerNameResolver;
...
import static io.kneo.qtracker.repository.table.OwnerNameResolver.OWNERS;

@ApplicationScoped
public class OwnerRepository extends AsyncRepository {
    private static final EntityData entityData = OwnerNameResolver.create().getEntityNames(OWNERS);
    ...
    public Uni<List<Owner>> getAll(final int limit, final int offset, final IUser user) {
        String sql = "SELECT * FROM __owners o, owners__readers orr WHERE o.id = orr.entity_id AND orr.reader = " + user.getId();
        if (limit > 0) {
    ...
    public Uni<Owner> insert(Owner doc, IUser user) {
        ...
        String sql = String.format("INSERT INTO %s " +
                "(reg_date, author, last_mod_date, last_mod_user, email, telegram_name, loc_name, phone, country, currency, status) " +
                ...
                .onItem().transformToUni(permissions -> {
                    if (permissions[0] == 1) {
                        ...
                        String sql = String.format("UPDATE %s SET last_mod_user=$1, last_mod_date=$2, email=$3, telegram_name=$4," +
                                " loc_name=$5, phone=$6, country=$7, currency=$8, status=$9 WHERE id=$10;", entityData.getTableName());
